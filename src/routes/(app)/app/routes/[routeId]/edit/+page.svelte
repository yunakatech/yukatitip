...
<svelte:head>
	<title>Ubah route | Yukatitip</title>
</svelte:head>

<section class="master-form-page">
	<PageHeader
		eyebrow="Data"
		title={`Ubah ${data.route.name}`}
		description="Perbarui route tanpa mengubah histori jadwal dan tarif yang sudah tersimpan."
	/>
		...
		<Card as="section">
			<div class="master-form__grid">
				<Select
					label="Origin"
					name="originBranchId"
					value={form?.values?.originBranchId ?? data.route.origin.id}
				...
				<Select
					...
					value={form?.values?.destinationBranchId ?? data.route.destination.id}
				...
				<Input
					label="Nama rute"
					name="name"
					value={form?.values?.name ?? data.route.name}
				...
				<Input
					...
					min="1"
					step="1"
					value={form?.values?.estimatedDurationMinutes ?? data.route.estimatedDurationMinutes ?? ''}
				...
				<Input
					...
					min="0"
					step="1"
					value={form?.values?.baseFee ?? data.route.baseFee}
				...
				<Select
					label="Status"
					name="isActive"
					value={form?.values?.isActive === false ? 'false' : data.route.isActive ? 'true' : 'false'}
