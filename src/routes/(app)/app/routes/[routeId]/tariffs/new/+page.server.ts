...
import { createRouteTariff, getRouteDetail, parseRouteTariffInput } from '$lib/server/master-data/routes';
...
export const load: PageServerLoad = async ({ locals, params }) => {
	...
	if (!route.isActive) {
		throw error(409, 'Route harus aktif sebelum tarif baru dapat dibuat.');
	}
...
export const actions: Actions = {
	default: async (event) => {
		...
		try {
			const route = await createRouteTariff(serviceClient, auth, params.routeId, input);

			await writeAuditLog(serviceClient, {
				requestId: locals.requestId,
				actorProfileId: auth.profile.id,
				action: 'route_tariff.created',
