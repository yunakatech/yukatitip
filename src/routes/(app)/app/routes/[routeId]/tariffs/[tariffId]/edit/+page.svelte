...
<svelte:head>
	<title>Ubah tarif | Yukatitip</title>
</svelte:head>

<section class="master-form-page">
	<PageHeader
		eyebrow="Data"
		title={`Ubah tarif ${ROUTE_SERVICE_LABELS[data.tariff.serviceType]}`}
		description="Perbarui tarif route tanpa menimpa histori efektif yang sudah tersimpan."
	/>
		...
		<Card as="section">
			<div class="master-form__grid">
				<Select
					label="Jenis layanan"
					name="serviceType"
					value={form?.values?.serviceType ?? data.tariff.serviceType}
				...
				<Input
					...
					min="0"
					step="1"
					value={form?.values?.minimumServiceFee ?? data.tariff.minimumServiceFee}
				...
				<Input
					...
					max="100"
					step="0.01"
					value={form?.values?.percentageFee ?? data.tariff.percentageFee}
				...
				<Input
					...
					min="0"
					step="1"
					value={form?.values?.localDeliveryFee ?? data.tariff.localDeliveryFee}
				...
				<Input
					...
					min="0"
					step="1"
					value={form?.values?.handlingFee ?? data.tariff.handlingFee}
				...
				<Input
					...
					name="effectiveFrom"
					type="date"
					value={form?.values?.effectiveFrom ?? data.tariff.effectiveFrom}
				...
				<Input
					...
					name="effectiveUntil"
					type="date"
					value={form?.values?.effectiveUntil ?? data.tariff.effectiveUntil ?? ''}
					error={form?.fieldErrors?.effectiveUntil?.[0]}
				/>
				<Select
					label="Status"
					name="isActive"
					value={form?.values?.isActive === false ? 'false' : data.tariff.isActive ? 'true' : 'false'}
